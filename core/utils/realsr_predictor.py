"""Changing the API of super resolution gan in PPGAN to make it support batch calculation."""
...
class RealSRPredictor(BasePredictor):
    ...
    def norm_images(self, images):
        images = np.array(images).transpose([0, 3, 1, 2]).astype("float32") / 255.0
        return images.astype("float32")

    def denorm_images(self, images):
        images = images.transpose((0, 2, 3, 1))
        return (images * 255).clip(0, 255).astype("uint8")

    def run_multiple_images(self, images):
        images = self.norm_images(images)
        x = paddle.to_tensor(images)
        with paddle.no_grad():
            out = self.model(x)

        pred_img = self.denorm_images(out.numpy())
        return pred_img
